...
// 是否开启 profiler（也可以用 -DLIO_SAM_ENABLE_PROFILING 在 CMake 里控制）
#define LIO_SAM_ENABLE_PROFILING 1

#if LIO_SAM_ENABLE_PROFILING

class LioSamProfiler
{
public:
    static LioSamProfiler& instance()
    {
        static LioSamProfiler inst;
        return inst;
    }

    // 在析构时调用：记录一条 (时间戳, 模块名, 耗时)
    inline void log(const char* name, double duration_ms)
    {
        std::lock_guard<std::mutex> lock(mutex_);
        if (!file_)
        {
            file_ = std::fopen("/home/niumu/ws_livox/src/LIO-SAM-MID360/data/liosam_profile.csv", "w");
            if (file_)
            {
                std::setvbuf(file_, buffer_, _IOFBF, sizeof(buffer_));
                std::fprintf(file_, "stamp_ns,module,duration_ms\n");
            }
        }
        if (!file_) return;

        const uint64_t stamp_ns = ros::Time::now().toNSec();
        std::fprintf(file_, "%lu,%s,%.3f\n",
                    (unsigned long)stamp_ns, name, duration_ms);
...
public:
    ...
    ~ScopedTimer()
    {
        ...
        LioSamProfiler::instance().log(name_, ms);
    }
...
#else  // LIO_SAM_ENABLE_PROFILING

// 关掉 profiler 时，宏为空，不产生任何开销
