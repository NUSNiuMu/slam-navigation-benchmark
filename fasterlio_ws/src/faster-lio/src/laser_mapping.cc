...
bool LaserMapping::LoadParams(ros::NodeHandle &nh) {
    ...
    nh.param<std::string>("common/map_frame", tf_world_frame_, tf_world_frame_);
    nh.param<std::string>("common/body_frame", tf_imu_frame_, tf_imu_frame_);
...
bool LaserMapping::LoadParamsFromYAML(const std::string &yaml_file) {
    ...
    try {
        ...
        if (yaml["common"]["map_frame"]) tf_world_frame_ = yaml["common"]["map_frame"].as<std::string>();
        if (yaml["common"]["body_frame"]) tf_imu_frame_ = yaml["common"]["body_frame"].as<std::string>();
...
void LaserMapping::SubAndPubToROS(ros::NodeHandle &nh) {
    ...
    if (preprocess_->GetLidarType() == LidarType::AVIA) {
        sub_pcl_ = nh.subscribe<livox_ros_driver::CustomMsg>(
            lid_topic, 200000, [this](const livox_ros_driver::CustomMsg::ConstPtr &msg) { LivoxPCLCallBack(msg); });
    } else {
        sub_pcl_ = nh.subscribe<sensor_msgs::PointCloud2>(
            lid_topic, 200000, [this](const sensor_msgs::PointCloud2::ConstPtr &msg) { StandardPCLCallBack(msg); });
    }
